...
This version compares set of trajectories vs 1 given struct, but for multiple subsequences.
'''
...
Sequences = ["r36","r37","r38","r39","r40","r41","r42","r43"]
FNclasses = [["b0","b1","b2","b3","b4"], ["c1","c2","c3"]] # "file name classes"
...
Descr += "Sequence classes: {}\n".format(Sequences)

Descr += '''If you need to change that, edit the code directly.

The output goes to fnbase+stub+seq_class".xvg".
...
def ProcessCommandLine():
    ...
    parser.add_argument('-grc', action='store_true', help="create the grace_str and grace_trj list files and the")
    parser.add_argument('-gay', action='store_true', help="autoscale y value for grace batch files (ignore value set in the script)")
...
for sq in Sequences:
    ...
    with open(args.fb+Tokens[0]+"-"+sq+".xvg") as F:
    ...
    for i in range(1,len(Tokens)):
        with open(args.fb+Tokens[i]+"-"+sq+".xvg") as F:
    ...
    with open(outName(sq), mode='w') as F:
        tabdata_io.write_xvg_gmx(data, F)
    #
    # grace list files
    if args.grc:
        if args.gay:
            grcA = 1.1 * data.max_data()
        FG.write("-graph {} -nxy {} -world 0 0 {} {} ".format(iG, outName(sq), grcT, grcA))
        iG += 1
